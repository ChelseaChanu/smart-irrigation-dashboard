...
function IrrigationForecast() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // Save tip and some weather info for Dashboard
      localStorage.setItem("aiForecastTip", mappedData.tip);
      localStorage.setItem("aiForecastTemp", mappedData.tempAvg);
      localStorage.setItem("aiForecastHumidity", mappedData.humidity);
      localStorage.setItem("aiForecastConditions", mappedData.conditions);
    ...
    <div className="min-h-screen bg-[#060C1A] text-white p-4 flex flex-col gap-6">
      ...
      <div className="flex flex-col md:flex-row justify-between mb-6 items-start">
        ...
        <div className="bg-gray-900 p-4 rounded">
          <h2 className="text-xl mb-2">Select Date (Next 14 days)</h2>
          <DatePicker
      ...
      {forecastData && (
        <div className="bg-gray-900 p-4 rounded">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div className="bg-gray-800 p-3 rounded text-center">
              <p className="font-bold">Max/Min Temp</p>
            ...
            <div className="bg-gray-800 p-3 rounded text-center">
              <p className="font-bold">Evapotranspiration</p>
              <p>{forecastData.evapotranspiration}</p>
            </div>
            <div className="bg-gray-800 p-3 rounded text-center">
              <p className="font-bold">Conditions</p>
              <p>{forecastData.conditions}</p>
            </div>
            <div className="bg-gray-800 p-3 rounded text-center">
              <p className="font-bold">Tip</p>
              <p>{forecastData.tip}</p>
            </div>
          </div>
        </div>
      )}
